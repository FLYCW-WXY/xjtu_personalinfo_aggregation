...
def getHTMLContent(url):
    try:
        ...
        return ""
def getXJTUEduNews(type):
    resultList=[]
    if type=='':
        s = BeautifulSoup(getHTMLContent('http://due.xjtu.edu.cn/jxxx/xytz.htm'),
                          'html.parser')  # 学籍管理、考试安排、课程安排、校际交流、竞赛安排、专业辅修、大创项目
    ...
    s=BeautifulSoup(getHTMLContent(edunewsbaseurl+type+'.htm'),'html.parser')#学籍管理、考试安排、课程安排、校际交流、竞赛安排、专业辅修、大创项目
    for div in s.find_all('div',{'class':'list-li'}):
